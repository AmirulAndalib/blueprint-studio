...
- **Long workspace values remain readable without shifting controls** — Long local and SFTP paths, file names, editor tabs, repository branches, host endpoints, terminal scopes, and translated workspace headings now use bounded ellipsis behavior. The shared tooltip owner reveals the complete value only when content is actually clipped, supports pointer and keyboard focus, and preserves full accessible names. SFTP breadcrumb segments also gain matching keyboard navigation semantics. The frontend asset version advances to 2.5.75.

- **Repository and connection context is consistent** — GitHub and Gitea panels now share a compact status indicator for uninitialized, local-only, and remote-connected repositories, including the active branch and the existing branch-manager action. SFTP uses the same visual language to show its selected endpoint and connecting or connected state without exposing authentication material. Full context is available to assistive technology and native tooltips without introducing decorative cards or extra workflow steps. The frontend asset version advances to 2.5.74.

- **The command palette now covers the complete command surface** — All 38 toolbar commands are discovered from their live controls and retain their existing handlers, while palette-only editor, Blueprint, theme, tab, help, and Developer Tools actions remain available. Command search now matches labels, functional scopes, shortcuts, and command metadata. Every result visibly reports its scope and availability; contextual or integration-disabled commands remain discoverable, explain why they are unavailable, expose matching accessibility state, and cannot be invoked until their requirement is met. The frontend asset version advances to 2.5.73.

- **Workspace proportions persist within usable limits** — Sidebar, AI Studio, terminal bottom-panel, and editor split proportions now restore through one remembered workspace-layout contract with explicit minimum and maximum bounds. Sidebar, terminal, and split separators support pointer and keyboard resizing with synchronized accessibility values, while viewport changes constrain the rendered dimensions without discarding the user's saved preference. Legacy width and split settings migrate transparently. Versioned layout consumers prevent older transitive modules from surviving an integration upgrade, and the public diagnostic state now references the same state instance used by the application. The frontend asset version advances to 2.5.72.

- **SFTP Explorer file rows and header actions render correctly again** — The remote connection and directory contents were loading successfully, but the file tree retained its pre-initialization hiding class after a connection became active. The SFTP renderer now releases that initial-only state before applying tree or navigation mode visibility. Both global and SFTP-specific translation refresh target only the connection label or default dropdown option instead of replacing the Add action's icon text with “SFTP” or “SFTP Connections.” Versioned SFTP consumers prevent the repaired renderer from remaining stale after upgrade. The frontend asset version advances to 2.5.70.

- **AI Studio sidebar visibility and sizing are reliable** — The existing assistant now uses deterministic open and close state, synchronizes desktop and mobile visibility classes, restores the toolbar trigger after closing, and exposes its expanded state to assistive technology. Desktop users can resize the panel from its left edge by pointer or keyboard within editor-preserving constraints, and the chosen width persists with workspace settings. Mobile retains its overlay width and swipe gestures without inheriting a desktop inline width. The active AI module boundary carries the release version so an older transitive module cannot survive an integration upgrade. The frontend asset version advances to 2.5.67.

- **Source Control, SFTP, and GitHub authentication titles resolve correctly** — Locale JSON requests now carry the integration asset version, preventing an older cached fallback bundle from rendering new keys such as `sidebar.source_control` or `sidebar.sftp` as visible text. Activity Rail labels retain their built-in readable names while a translation bundle is unavailable, and the visible SFTP panel heading now uses the same localized label. All ten shipped locale bundles include Source Control, SFTP, and GitHub authentication labels. The GitHub OAuth device-flow dialog now uses its own “Sign in to GitHub” title instead of the Gitea authentication title. The frontend asset version advances to 2.5.65.

- **Terminal and Developer Tools panel states are clearer** — The existing terminal now presents explicit Local or SSH host scope, persistent connecting/connected/reconnecting/disconnected/error status, and an accessible reconnect command in a stable responsive header. Its bottom-panel and editor-tab modes retain the existing emulator and transport while sharing named presentation classes, preserving a resized panel height across mode changes, restoring a persisted visible panel without a second click, and preventing an intentionally disabled terminal from scheduling a new connection. Developer Tools retains its existing Actions, Template, States, and Config panes while adding proper tablist, tab, and tabpanel semantics, roving focus, Arrow/Home/End navigation, a named icon close command, and accessible form controls. The frontend asset version advances to 2.5.63.

- **Activity Rail modes and states are consistent** — Source Control is now a dedicated Activity Rail destination that hosts the existing Git and Gitea panels without replacing their actions or settings. Explorer, Search, Source Control, and SFTP share selected, focus, hover, loading, empty, ready, and unavailable state styling. Source Control aggregates enabled Git/Gitea change counts into an accessible badge, while disabled Source Control and SFTP integrations remain discoverable with clear Settings-oriented unavailable states. The frontend asset version advances to 2.5.61.

- **Toolbar overflow follows command priority** — The workspace toolbar now measures its available width and moves lower-priority functional groups into a shared More Commands menu instead of requiring horizontal scrolling or arbitrarily removing five controls on phones. All 38 original commands remain reachable by their existing IDs and handlers; displaced commands retain icons, labels, shortcuts, enabled state, and disabled explanations. The menu supports Arrow Up/Down, Home, End, Escape, outside dismissal, focus restoration, and responsive repositioning. Feature-hidden commands remain hidden until their owning feature enables them. The frontend asset version advances to 2.5.60.

- **Toolbar tooltips and availability are consistent** — All 38 workspace toolbar commands now use the shared tooltip service and synchronized accessible names instead of a mixture of one shared tooltip and native browser titles. Translation, Show Hidden, and One Tab Mode updates feed the same label owner. Save, Save All, Undo, Redo, and Download retain their existing enabled-state behavior while exposing a specific unavailable reason through tooltip and accessibility text. Disabled controls can show that reason on pointer hover. The frontend asset version advances to 2.5.59.

- **Workspace toolbar grouping started** — Phase 2 begins by assigning accessible functional scopes to every existing toolbar command cluster. Support, workspace tools, Home Assistant instance operations, and application controls are no longer presented as one mixed group. Restart and Developer Tools retain their existing handlers and access while moving into a clearly labelled instance-control boundary with restrained warning styling; every toolbar command remains available in its prior order. The frontend asset version advances to 2.5.58.

- **Large multipart ZIP uploads no longer receive an immediate 413** — The upload view now uses `sys.maxsize` as aiohttp's effectively unlimited multipart limit. Zero was incorrectly interpreted as a literal zero-byte body-part limit, so large drag-and-drop ZIP uploads were rejected before the disk-backed spool could read their first chunk. A 2.43 GiB ZIP upload now completes successfully through the deployed browser workflow.

- **Frontend asset cache advanced to 2.5.57** — Phase 1 is complete. The integration version advances the existing `?v={{VERSION}}` asset contract so deployed foundation changes replace cached resources consistently. Every first-party Material Icon now consumes the shared `ui-icon` wrapper without inline presentation, and repository-wide contracts prevent either unwrapped icons or inline icon sizing and coloring from returning. The final 53 declarations across onboarding, GitHub/Gitea integration, branch management, asset previews, tabs, terminal, Explorer, user guide, Blueprint forms, and diff actions now use shared size, semantic tone, alignment, spacing, or feature-owned state classes. Earlier Settings, Developer Tools, Git, and Gitea panel migrations retain their established dimensions and semantic or brand tones. The shared primitive, dialog-manager, tooltip, and feedback-service boundaries now have one documented interaction ownership contract. A standalone, backend-independent component showcase continues to render production tokens, controls, navigation, feedback, progress, loading, empty, notification, tooltip, and dialog states for deterministic light/dark desktop and mobile visual testing without starting the editor.

- **Frontend design-system foundations started** — Added Home Assistant-aware semantic surface, content, border, focus, status, typography, spacing, sizing, radius, shadow, and stacking tokens plus shared control, icon, segmented, menu, tooltip, dialog, panel, feedback, progress, skeleton, empty-state, alert, and toast primitives. Representative editor and Explorer toolbar commands, including Toggle Files, Save All, Format Code, Split Editor, Close Split View, New File, New Folder, Show Hidden, Select Files, Collapse All Folders, One Tab Mode, Use Blueprint, file/folder transfer actions, selection actions, Copy Breadcrumb, Markdown Preview, Activity Rail navigation, Explorer search/filter and folder-navigation controls, Favorites file and unpin glyphs, Recent Files file glyph, transfer-progress glyphs, centralized toast status and dismiss glyphs, command-palette command, file, and mobile-launcher glyphs, autocomplete snippet glyphs, entity-inspection fallback and Copy ID glyphs, shared AI and Markdown code-copy glyphs, Git/Gitea panel actions, SFTP/Search sidebar header icons, editor Find/Replace disclosures, navigation, close, and replacement actions, the Global Search Replace disclosure, Replace All action, loading spinner, and empty-state glyph, status-bar connection, theme trigger, preset menu, and indentation-picker glyphs, Keyboard Shortcuts and standard modal close controls, welcome-screen identity and action glyphs, Support-dialog option and action glyphs, and Donation-dialog brand, external-link, and copy glyphs, plus tooltip and right-side utility controls, colored operational toolbar icons, the theme preset menu, central modal, Explorer panel and controls, Global Search scope tabs, startup loading, file-tree skeletons, empty states, and centralized toast creation now consume the shared compatibility classes without changing their existing layout or theme behavior. Empty editor Replace-row spacers no longer advertise the Material Icons font. AI Studio, Restart Home Assistant, Developer Tools, Gitea Settings, and static Gitea panel brand icons now use shared semantic tones instead of inline color declarations.

- **AI UI ownership consolidated** — Removed the unused legacy `ai.js` implementation. The active AI sidebar, message history, provider request flow, and code-copy behavior now have one documented frontend owner in `ai-ui.js`, with runtime consumers and regression coverage enforcing that boundary.

- **AI UI presentation classes centralized** — AI toolbar visibility, rendered Markdown response framing, and provider error tone now use named component and state classes instead of runtime or markup presentation styles, preserving the existing enabled/disabled layout and theme behavior.

- **Primary navigation layout classes added** — The Activity Rail spacer and Explorer file-tree collapse alignment now use named layout classes instead of inline flex declarations, preserving the existing rail distribution and folder-navigation geometry.

- **Global Search scope presentation centralized** — The All, Files, and Entities scope control now derives its sizing, spacing, and selected-state colors from named classes. Tab activation updates semantic state only and no longer writes presentation styles into the DOM.

- **Global Search query layout centralized** — The primary query row, input wrapper, query-field inset, and search-modifier group now use named component classes instead of inline layout declarations.

- **Global Search disclosures centralized** — Replace and include/exclude panels now use named layout classes and a shared expanded state. Pointer and keyboard disclosure changes no longer write display styles into the DOM.

- **Global Search static frame centralized** — The search-panel spacing, results scroller, and initial empty-state layout now use named component classes instead of inline presentation declarations.

- **Global Search runtime states centralized** — Dynamically rendered empty, no-result, and failure states now use named component classes and shared icon wrappers instead of inline presentation declarations; backend failure messages are HTML-escaped before rendering.

- **Global Search file-result presentation centralized** — Dynamically rendered file headers, folder labels, action groups, count badges, disclosure glyphs, and expanded result lists now use named component classes and shared icon wrappers instead of inline presentation declarations.

- **Global Search match-row presentation centralized** — Dynamically rendered match rows, line numbers, excerpts, hover actions, and replace/dismiss glyphs now use named component classes and shared icon wrappers instead of inline presentation declarations.

- **Global Search entity-result presentation centralized** — Dynamically rendered entity headers, count badges, result lists, names, and IDs now use named component classes and shared icon wrappers instead of inline presentation declarations, and displayed entity data is HTML-escaped.

- **Global Search loading state centralized** — Search activity now toggles one semantic spinner state class and synchronized `aria-hidden` state instead of writing display styles directly.

- **Clipboard fallback and entity popup presentation centralized** — Global Search and editor entity copying now share the established clipboard utility, its temporary control uses a named off-screen class, and the entity popup's static frame and MDI icon presentation moved from runtime `cssText` into component CSS. Only click-dependent popup coordinates remain dynamic.

- **Support and Donation dialog presentation centralized** — Both dialog widths plus Support option, icon-tone, title, description, link-grid, action-link, and footer presentation now use named component classes instead of static inline declarations.

- **Frontend keyboard and screen-reader foundations improved** — Blueprint Studio now exposes named workspace landmarks, accessible file search, keyboard-operable activity rail and sidebar controls, an announced command-palette listbox, keyboard navigation for Explorer and editor breadcrumbs, and explicitly named AI message, Close, and Send controls. File-tree, Global Search, and primary/split-editor Find/Replace disclosures expose their controlled regions and keep expanded state synchronized across pointer, keyboard, command, and shortcut entry points. Global Search and editor search modifiers now announce their pressed state and keep shared editor options synchronized across panes. The Global Search scope selector supports a single Tab stop plus Left/Right/Home/End navigation, while the exclusive theme preset menu announces checked state, supports arrow/Home/End navigation and Enter/Space selection, and restores trigger focus after Escape. Automated accessibility audits remain at zero violations, and focused CodeMirror regression contracts and desktop/tablet/phone captures prevent the full-width blue-line artifact from being reintroduced.
- **Keyboard Shortcuts dialog semantics completed** — The shortcuts overlay now exposes modal-dialog and labelled-title semantics, while its close control has an explicit accessible name and button type.

- **Home Assistant YAML autocomplete is broader and context-aware** — Autocomplete now recognises standalone automation files, automation blueprints, modern plural `triggers:`, `conditions:`, and `actions:` blocks, and nested trigger, condition, target, action, and service-data contexts. Typed prefixes are read directly from the text before the cursor and matched from the beginning of each candidate, preventing unrelated and duplicate suggestions. The built-in catalog now covers common Home Assistant trigger, condition, script-control, wait, repeat, event, response, error-handling, device, target, and blueprint keys, including `entity_id:` wherever it is valid, while retaining legacy singular and `platform:`/`service:` syntax. Entity completion loads the full entity set instead of stopping at 1,000; registered actions and their selector-aware fields come from the running Home Assistant instance; and `entity_id`, `device_id`, `area_id`, `floor_id`, `label_id`, state, and action-field values are populated from live states and registries. Selected device automation blocks lazily load integration-provided triggers, conditions, and actions and derive their `domain`, `type`, `subtype`, and other scalar values without scanning every device at startup. Blueprint `!input` discovery supports flexible indentation, autocomplete metadata failures are reported, and live names, icons, and descriptions are rendered without unsafe HTML interpolation. Regression coverage now exercises prefix filtering, deduplication, standalone and blueprint context detection, nested YAML paths, action-field ancestry, device sibling context, transport contracts, and catalog guarantees.

- **Backend quality gates and operations documentation** — CI now runs the backend unit and Home Assistant fixture suites, Ruff formatting and linting, mypy, bytecode compilation, Hassfest, and HACS validation. Generated-stream tests simulate multi-gigabyte transfers with bounded memory, while regression coverage exercises traversal and symlink escape, ticket replay, Git admission/time bounds, lifecycle cleanup, and every historical storage shape. Architecture, security, troubleshooting, and contributor guidance now describe runtime ownership and operational limits.

- **Redacted diagnostics and bounded observability state** — Home Assistant issue-report diagnostics now expose manager readiness, active request and transfer counts, queued work, terminal sessions, pooled connection counts, and transient registry sizes without including paths, file content, users, tokens, or credentials. HTTP operations emit consistent correlation-aware result and duration logs. Progress records expire five minutes after completion, stale active records expire after one hour, and progress, stream, download, ticket, and grant registries have hard size limits.

- **Storage upgrades are now explicit and lossless** — Blueprint Studio uses a typed version-2 Home Assistant store with an atomic, idempotent migration from every historical version-1 shape. Workspace fields, unsaved buffers, hosts, credentials, and unknown future fields are preserved; failed validation leaves the original store untouched. Paramiko is pinned to the version tested with Home Assistant, and the documented minimum Home Assistant release is now 2024.7.

- **Cloud AI model lists now stay current automatically** — Opening cloud AI settings and switching providers now refreshes available models from Gemini, OpenAI-compatible, or Anthropic model APIs through the authenticated backend. Static cloud model catalogs are no longer treated as authoritative, while a configured custom model remains selectable.

- **Long-running backend work is bounded and cancellable** — Repository mutations now serialize to prevent concurrent Git state changes, while Git reads, SFTP calls, searches, terminal commands, and other expensive operations use runtime-owned concurrency limits. Hung non-transfer operations return a timeout response, AI providers reuse Home Assistant's shared HTTP session, and SFTP connection phases have explicit deadlines. Active uploads and downloads retain unlimited duration while progress continues.

- **Backend requests now use typed transport contracts** — Every REST, SFTP, streaming, multipart upload, Home Assistant WebSocket, and terminal control action is validated consistently before dispatch. Required fields, booleans, lists, dictionaries, ports, modes, providers, branch names, and terminal dimensions reject malformed values through the existing response envelope, while stable internal error categories and per-request correlation IDs improve safe troubleshooting without changing frontend response fields.

- **Large transfers now use constant memory** — Multipart uploads spool to disk in fixed-size chunks, local files finalize atomically, SFTP uploads write directly through bounded buffers, and ZIP uploads extract incrementally from disk-backed spools. Cancellation, disconnect, and failure paths remove partial files and temporary data, while bounded download queues preserve backpressure and existing progress, merge, replace, overwrite, and ZIP safety behavior.

- **Stream and terminal URLs no longer expose Home Assistant tokens** — Admin-authenticated requests now issue short-lived, single-use tickets scoped to the user, operation, and exact resource. Stream tickets exchange atomically for expiring scoped grants that preserve media Range requests, while terminal tickets require a same-origin WebSocket handshake. Expired, replayed, mismatched, inactive-user, and non-admin authorization is rejected, and all outstanding tickets are cleared on unload.

- **Backend lifecycle and reload reliability improved** — Blueprint Studio now owns its managers, persistent storage, event subscriptions, and deferred background work through a typed config-entry runtime. Global HTTP routes, WebSocket commands, static assets, and panel registration are idempotent across integration reloads; routes rebind to the current runtime and return a service-unavailable response while the entry is unloaded instead of retaining stale managers.

- **Entry unload now releases tracked resources** — Folder-watcher subscriptions and deferred Git status tasks are retained as entry-owned resources and cancelled or unsubscribed during unload. Pooled SFTP connections, terminal WebSockets, PTYs, event-loop readers, and local or SFTP ZIP producer threads are also closed. New backend requests are rejected once shutdown starts; active HTTP operations receive a bounded drain period before remaining work is cancelled and awaited. Transfer workers are explicitly signalled and joined, blocked ZIP queue waits remain cancellable, and transient stream tokens and progress records are cleared. Existing workspace storage and frontend request/response contracts remain unchanged.

- **Automation-to-blueprint conversion handles more valid YAML forms** — Quoted entity IDs and flow-style entity lists are now extracted correctly, quoted entity values are converted to valid unquoted `!input` tags, and add-on inputs use Home Assistant's app selector.
...
- **Manage Git Exclusions reopens with wrong checkbox states** — The `read_file` API response does not include a `success` field, so the check `if (response.success)` always evaluated to `undefined` (falsy), leaving `gitignoreContent` empty every time. The UI then fell back to four hardcoded defaults (`.cloud`, `.storage`, `deps`, `.ha_run.lock`) regardless of what `.gitignore` actually contained. The check now correctly reads `response.content`. Additionally, files covered by glob patterns (e.g. `*.db`, `*.db-shm`) were always shown as checked (included) because the `isPathIgnored` helper only performed exact string matches. It now evaluates wildcard patterns so files matched by a glob correctly appear unchecked (excluded) when the dialog is reopened.
...
- **Status bar and settings panel out of sync** — Changing indentation via the status bar picker did not update the Settings panel dropdown/checkbox, and vice versa. Both now stay in sync: the status bar picker updates the settings panel DOM elements, and the settings panel handlers call `updateStatusBar()` after saving.

- **`indentWithTabs` ignored when applying indentation** — When "Indent with Tabs" was enabled, `indentUnit` was incorrectly set to `tabSize` (e.g. 4) instead of `1`. CodeMirror's `indentUnit` must be `1` when using tabs so that `indentSelection` adds/removes one tab character per level. Fixed in `applyEditorSettings`, both settings panel handlers, the status bar picker, and `createEditor`.

- **Opening a file overwrote global indent preference** — Per-file indentation detection was writing the detected `tabSize` and `indentWithTabs` back to `state`, so switching files would permanently change the user's saved preference for the session. Detection now sets editor options directly without touching `state`.

- **`applyEditorSettings` stomping per-file indent on visibility change** — When switching back to the browser tab, `settings:loaded` fired and `applyEditorSettings` reset the editor to the global preference, overriding the per-file detected or `.editorconfig`-specified indentation. Indent options are now skipped in `applyEditorSettings` when a file is already open.

- **Indentation auto-detection producing "Spaces 1"** — The previous detection algorithm counted absolute indent depths, which caused GCD to collapse to 1 on files with YAML block scalars or multi-level jumps. Detection now uses frequency-filtered GCD of indent *changes* between consecutive lines, and excludes size `1` as a valid result. Single-occurrence deltas (e.g. block scalar continuations) are ignored.

- **Tab-indented files defaulting to size 4 regardless of user preference** — When a file was detected as tab-indented, `tabSize` was hardcoded to 4. It now uses `state.tabSize` (the user's saved preference).

- **`activateTab` not declared `async`** — Adding `await getEditorConfigIndent(...)` inside `activateTab` caused a `SyntaxError: Unexpected reserved word` crash on load. `activateTab` is now correctly declared `async`.
...
- **Developer Tools panel** — A native floating Developer Tools panel (toolbar button + command palette) brings HA's developer tools experience inside Blueprint Studio without leaving the editor. Four tabs:
  ...
  - **States** — Full entity state browser with domain filter and text search. Fetches real HA attributes (`with_attributes: true`); the Attributes column shows a short inline summary and clicking any row expands the full attribute table.
  - **Config** — HA configuration check (same validator as the removed toolbar button) plus one-click YAML reload buttons for 14 domains (automations, scripts, scenes, groups, all input helpers, timers, counters, schedules, template entities, and "Reload all YAML").
  - The panel is draggable by its header, dismissible via ✕ or Escape, and can be opened to a specific tab from the command palette ("Developer Tools: Actions / Template / States / Config").
...
[2.4.6]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.4.6
[2.4.5]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.4.5
[2.4.4]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.4.4
[2.4.3]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.4.3
[2.4.2]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.4.2
[2.4.1]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.4.1
[2.4.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.4.0
[2.3.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.3.0
[2.2.2]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.2.2
[2.2.1]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.2.1
[2.2.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.2.0
[2.1.5]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.1.5
[2.1.4]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.1.4
[2.1.3]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.1.3
[2.1.2]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.1.2
[2.1.1]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.1.1
[2.1.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.1.0
[2.0.6]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.0.6
[2.0.5]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.0.5
[2.0.4]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.0.4
[2.0.3]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.0.3
[2.0.2]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.0.2
[2.0.1]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.0.1
[2.0.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v2.0.0
[1.5.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v1.5.0
[1.4.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v1.4.0
[1.2.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v1.2.0
[1.0.0]: https://github.com/ha-china/blueprint-studio/releases/tag/v1.0.0
