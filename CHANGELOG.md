...
- **A more usable workspace** - Explorer, Search, Source Control, SFTP, the editor, terminals, Developer Tools, and AI Studio now work consistently across desktop, tablet, and phone layouts.
- **Safer editing** - File creation and renaming preserve your text and cursor position. Save, Save All, formatting, validation, transfers, and remote edits now report clear progress and recoverable errors.
- **Better Home Assistant support** - YAML completion, validation, entity and device pickers, action field details, Blueprint guidance, and current Home Assistant syntax are grounded in the connected instance when available.
- **Reviewed AI changes** - Ask, Explain, Generate, Fix, and Refactor modes use the active file or selection as context. Generated edits appear as reviewable diffs and require explicit approval before files change.
- **More reliable Git workflows** - GitHub and Gitea settings share a consistent design, authentication status is clearer, repository actions explain failures, and changes can be reviewed before applying them.
- **Improved operations feedback** - Long-running work appears in the Operations center with scope, progress, full error details, Retry, Open, and cancellation actions. The center can start minimized or be disabled in Settings.
- **Accessibility and localization** - Native controls, keyboard navigation, focus restoration, tooltips, live status announcements, contrast checks, RTL support, pseudo-localization, and translated workflow messages are covered throughout the workspace.
- **Responsive and performance improvements** - Phone navigation, touch targets, dialogs, status bars, large workspaces, lazy-loaded previews, and startup behavior have been tested against the live Home Assistant panel.

### Backend Reliability and Security

- **Large transfers use steady memory** - Uploads, downloads, ZIP files, and SFTP transfers stream in bounded chunks, report progress, clean up cancelled or failed work, and do not impose a Blueprint Studio file-size limit.
- **Reloads clean up correctly** - Reloading the Home Assistant integration closes its owned listeners, transfers, terminal sessions, SFTP connections, temporary files, and background tasks before starting again.
- **Connections are safer** - Streaming and terminal access use short-lived, scoped tickets instead of reusable Home Assistant tokens in URLs. Expired or replayed tickets are rejected, and credentials are removed from logs and diagnostics.
- **Requests fail clearly and consistently** - Backend actions validate their inputs before work begins and return actionable errors for invalid paths, conflicts, unavailable services, timeouts, cancellation, permissions, and connection failures.
- **Operations are easier to diagnose** - Long-running work has correlation details, bounded progress history, and redacted diagnostics without exposing file contents, tokens, keys, or passwords.

### Editor and AI Intelligence

- **Autocomplete follows the document** - Suggestions understand configuration files, automations, scripts, scenes, packages, Blueprints, nested actions, triggers, conditions, selectors, Jinja, and incomplete YAML instead of relying only on indentation guesses.
- **Live Home Assistant data improves suggestions** - Installed actions, fields, entities, devices, areas, labels, floors, selector types, custom domains, the running HA version, and capability information are used when available, with clear fallback states when they are not.
- **Validation explains why a finding exists** - Problems identify whether a result comes from syntax, schema, the running instance, a configuration check, or an optional recommendation. Valid files are not marked invalid just because they could be styled differently.
- **AI context is visible and minimized** - Before sending a request, users can see the provider class, active file or selection, included metadata, and redaction count. Unrelated files and sensitive values are excluded from provider requests.
- **AI requests recover safely** - Requests support cancellation, timeouts, bounded responses, retry, malformed-output handling, and provider-specific model or endpoint settings without silently changing files.

### Notes for Existing Users

- Existing files, settings, workspace layout, open tabs, and provider credentials are preserved during the upgrade.
- Cloud, local, or Home Assistant AI providers remain optional. Rule-based assistance continues to work without an external provider.
- AI-generated file changes are never applied automatically; review the diff and choose which changes to apply.
...
[Unreleased]: https://github.com/ha-china/blueprint-studio/compare/v3.0.0...HEAD
